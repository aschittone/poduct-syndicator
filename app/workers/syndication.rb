...
# calls to additional APIS can be made below, to syndicate to additional sites:
  ...
  def perform
		...
		# additional info can be included to create event
			Excon.post(`https://www.eventbriteapi.com/v3/events?event.name.html=#{product.name}event.listed=true&event.start.timezone=EST&event.start.utc=#{product.start_time}&event.end.utc=#{product.end_time}&event.end.timezone=EST&event.currency=USD&event.online_event=true`,
				:headers => { 
